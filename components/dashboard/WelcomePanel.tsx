          ...
          {welcomeCards.map((card) => {
            ...
            return (
              <Link
                key={card.title}
                href={card.href}
                className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm transition-colors hover:border-amber-200/60 hover:bg-[#FAF8F4]"
              >
                <Icon size={18} className="text-[#C9922A]" />
                <p className="mt-3 text-base font-black text-[#080A0F]">{card.title}</p>
