...
| `content-roadmap.csv` | Hoja de ruta de contenido: nuevas páginas y artículos a crear con keyword, prioridad y fecha objetivo |
...
## content-roadmap.csv — Guía de columnas

| Columna | Descripción | Valores posibles |
|---------|-------------|-----------------|
| ID | Identificador numérico único (001, 002...) | 001–999 |
| Tema | Título editorial del contenido a crear | Texto libre |
| Keyword principal | Término principal de búsqueda objetivo | `cómo conseguir reseñas google barbería` |
| Keyword secundaria | Término secundario complementario | Texto libre |
| Tipo de página | Categoría funcional del contenido | Landing SEO, Landing ciudad, Landing comparativa, Blog — Guía informacional, Caso de estudio |
| Ciudad | Ciudad target si es landing local | `Madrid`, `Valencia`, vacío si es nacional |
| Oferta conectada | Producto o servicio al que sirve este contenido | `BarberíaOS SaaS`, `Directorio público`, `Movimiento 1000 Barberías` |
| Prioridad | Importancia para SEO y conversión | Alta, Media, Baja |
| Estado | Estado de desarrollo del contenido | Pendiente — CREAR, En sitio — [URL], En redacción, Publicado |
| Fecha objetivo | Fecha estimada de publicación | `2026-06-15` |
| Notas | Contexto editorial, técnico o de negocio | Texto libre |

### Cómo usar el content-roadmap

**Para priorizar qué crear primero:**
1. Filtra `Prioridad` = `Alta` + `Estado` = `Pendiente — CREAR`
2. Empieza por los tipos `Landing SEO` y `Landing comparativa` — captan tráfico transaccional inmediato

**Para planificar el mes:**
1. Filtra `Fecha objetivo` del mes en curso
2. Revisa `Tipo de página` — equilibra landings (conversión) y guías (tráfico informacional)
3. Cuando publiques, actualiza `Estado` a `En sitio — /ruta-publicada`

**Para conectar con pages-inventory.csv:**
- Cuando crees la página, añade una fila nueva en `pages-inventory.csv` con los datos SEO completos
- El campo `Notas` del content-roadmap incluye el ID del pages-inventory cuando ya existe

---

## Cómo conectar con Google Search Console

Una vez desplegado en producción:

1. **Verificar propiedad**: ve a [search.google.com/search-console](https://search.google.com/search-console) → Añadir propiedad → tipo URL
2. **Verificación recomendada**: descarga el archivo HTML que te dan y colócalo en `public/` (Next.js lo sirve automáticamente)
3. **Enviar sitemap**: en Search Console → Sitemaps → añade `https://tu-dominio.com/sitemap.xml`
4. **Esperar indexación**: Google tarda 1-4 semanas en rastrear y posicionar páginas nuevas
5. **Rellenar datos en el CSV**: mensualmente, exporta desde Search Console → Rendimiento → por páginas y copia Clicks, Impresiones, CTR y Posición en `pages-inventory.csv`

**Filtros útiles en Search Console:**
- Filtra por página para ver rendimiento individual
- Filtra por país = España para datos locales
- Compara periodos mensuales para detectar tendencias
...
| `content-roadmap.csv` | Hoja de ruta editorial — complementa este inventario con contenido aún no creado |
